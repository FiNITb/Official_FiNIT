...
const Homepage = () => {
  ...
  useGSAP(() => {
    ...
    const letters = [...titleText].map((letter, index) => {
    ...
    tl.to(sideImgRef.current, { opacity: 1, x: 0, duration: 1.2 })
      ...
      .to(letters.reverse(), { opacity: 0, y: 50, duration: 0.8, stagger: 0.15 }, "+=1");

    gsap.to(heroImgRef.current, { y: 15, duration: 2, repeat: -1, yoyo: true, ease: "sine.inOut" });

    gsap.to(glowRef.current, { scale: 1.1, opacity: 0.7, duration: 3, repeat: -1, yoyo: true, ease: "sine.inOut" });
  }, []);
  ...
  return (
    <>
      <img ref={sideImgRef} src={side} alt="Side Design" className="fixed max-w-[200px] md:max-w-[300px]" />
        ...
        <div className="max-w-7xl w-full flex flex-col md:flex-row gap-10 items-center text-white z-10">
          ...
          <div className="text-center md:text-left space-y-2">
            ...
            <div className="min-h-[50px]">
              {!hideTitle && (
                <h1 ref={titleRef} className="font-bold leading-tight font-[Poppins]" style={{ fontSize: "clamp(2rem, 5vw, 6rem)" }}>
                  FiNIT
                </h1>
              )}
            ...
            <p ref={subtitleRef} className="font-bold mt-4 mb-3 leading-snug uppercase font-[Poppins]" style={{ fontSize: "clamp(1.5rem, 4vw, 3rem)" }}>
              <span className="text-teal-500">The Finance Society</span> <br />
              NIT Bhopal
            </p>

            <span ref={sloganRef} className="font-semibold py-4 block" style={{ fontSize: "clamp(1rem, 2.5vw, 1.75rem)" }}>
              where innovation meets <span className="text-[#9b51e0]">financial literacy</span>.
            </span>
            ...
            <div className="mt-6">
              <a
                ...
                className="px-6 py-3 bg-teal-500 text-white rounded-full font-semibold transition inline-flex justify-center items-center text-lg sm:text-xl"
          ...
          <div className="relative flex justify-center w-full md:w-auto">
            <div ref={glowRef} className="absolute inset-0 flex justify-center items-center">
              <div className="w-[80%] h-[80%] bg-purple-500 blur-3xl opacity-50 rounded-full"></div>
            </div>
            <img ref={heroImgRef} src={heroImg} alt="Crypto Illustration" className="relative max-w-full sm:max-w-[400px] md:max-w-[500px] lg:max-w-[600px] h-auto" />
          </div>
        </div>
      </div>
    </>
